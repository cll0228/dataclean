        ...
        if (!"".equals(route.getItinerarydetails()) && null != route.getDestination() && "跟团游".equals(route.getTeamtype())) {
            ...
            try {
                switch (route.getDatasource()) {
                    case "Lvmama":
                        tripEntityList = lvmamaTripRoute(route);
                        break;
                    case "Ctrip":
                        tripEntityList = ctripTripRoute(route);
                        break;
                    case "Tongcheng":
                        tripEntityList = praseTongCheng(route);
                        break;
                    case "Tuniu":
                        tripEntityList = praseTuniuRoute(route);
                        break;
                    case "Qunaer":
                        tripEntityList = qunaerRoutePrase(route);
                        break;
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public static List<TripEntity> lvmamaTripRoute(Routeinfo routeinfoDto) {
        ...
        try {
            ...
            for (int i = 0; i < size; i++) {
                TripEntity tripEntity = tripEntities.get(i);
                String hotel = "";
                if (i < ele_ul.size() - 1) {
