...
import util.DataStandardFunction;
import util.StandardUtil;

import java.util.*;

public class RouteCleanStartor {

    private static final Logger logger = LoggerFactory.getLogger(RouteCleanStartor.class);

    public static void main(String[] args) {
        SparkConf conf = new SparkConf().setAppName("route_clean");
        if (args.length == 0) {
        ...
        Collection<String> topics = Arrays.asList("topic_route");
        ...
        dataJavaDStream.foreachRDD((JavaRDD<ReceiverData> robotObjectJavaRDD) -> {
            ...
            if (null != list && list.size() != 0) {
                for (ReceiverData obj : list) {
                    RouteObjectDao.saveToHbase(obj);
                }
            }
        });
